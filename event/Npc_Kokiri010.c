...
Actor: Npc_Kokiri010[Before_killedEnemy]
entrypoint: None()
...
void Talk() {
    ...
    if !GameDataActor.EventQueryIsGameDataBool({'Index': -1, 'GameDataName': 'Order_kokiri010'}) {
        if Npc_Kokiri010[AfterChallenge].EventQueryIsOnRecentTalkFlag() {
...
void EnemyKilled_AccidentOfDekutree_Talk() {
    ...
    Npc_Kokiri010[Before_killedEnemy].EventTalk({'MessageId': 'EventFlowMsg/Npc_Kokiri010:Talk1001', 'IsWaitEnd': True, 'IsCloseDialog': True, 'EndDialogOption': 0, 'ASName': '', 'IsNotBecomeSpeaker': False, 'ChangeMstxtForActor': False, 'ChoiceNumber': 0, 'ChoiceLabel1': 0, 'ChoiceLabel2': 0, 'ChoiceLabel3': 0, 'ChoiceLabel4': 0, 'ChoiceDefaultNo': 0, 'ChoiceCancelNo': 0, 'IsAnimeDriven': False, 'IsOverrideStyle': False, 'OverrideStyle': 0, 'IsPlayASAllSlot': False, 'NotPlayLipSyncAnim': False, 'IsAnonymous': True, 'IsNotOpenIfSkipedMostRecent': False})
    Npc_EventStarter.EventTriggerSetHaveTalked({'value': False, 'IsReflectWhenSleep': False})
}

void EnemyKilled_AccidentOfDekutree_Near() {
    Npc_Kokiri010[Before_killedEnemy].EventTriggerBalloonMessage({'IsChallenge': False, 'DisplayFrame': 90, 'IsIgnoreDisplayKeepDepth': False, 'MessageID': 'EventFlowMsg/Npc_Kokiri010:Near1001', 'IsDisplayInTerror': False, 'NotOverriddenByInterestBalloon': False})
}
